...
export function Header() {
  ...
  return (
    <header className="fixed top-4 left-1/2 -translate-x-1/2 z-50 w-[95%] max-w-7xl">
      <div className="bg-black/90 backdrop-blur-2xl rounded-full border border-white/20 shadow-[0_8px_32px_rgba(0,0,0,0.4)] px-4 md:px-6 py-3 md:py-4 flex justify-between items-center transition-all duration-500 hover:shadow-[0_12px_48px_rgba(0,0,0,0.6)]">
        <Link to="/" className="flex items-center gap-3 cursor-pointer group">
          ...
          <div className="hidden md:block">
            <div className="font-semibold text-sm md:text-base tracking-tight leading-tight text-white group-hover:text-secondary transition-colors">
              Wells Fargo Business Credit Close-Up
            </div>
            <div className="text-[10px] text-white/70">Powered by LUMIQ AI</div>
          </div>
        </Link>
        ...
        <nav className="hidden lg:flex items-center gap-0.5">
          {navLinks.map((link) => (
            <Link
              key={link.to}
              to={link.to}
              className="px-3 py-2 text-sm font-medium text-white/70 hover:text-white hover:bg-white/10 rounded-full transition-all"
            >
              {link.label}
            </Link>
          ))}
